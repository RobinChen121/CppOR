 ...
 * Description:
 *  test the influence of the number of sampled scenarios in the forward pass.
 *
 *
 */
...
int main() {
  ...
  double opt = 167.38;
  for (int i = 0; i < 1; i++) {
    for (int forward_num : forward_nums) {
      for (int n = 0; n < runs; n++) {
        //            const auto &demands = demands_all[i];
        auto problem =
            SingleProduct(demands, price, interest, overhead, sampleNum, forward_num, iter_num);
