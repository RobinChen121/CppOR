...
public:
  ...
  bool simplexPhase(const std::vector<double> &phase_c, const std::vector<bool> &allow_enter,
                    BasisFactorization &factor, int max_iter, double &phase_obj);
...
bool LinearModel::solvePrimalSimplex() {
  ...
  if (!simplexPhase(c_phase1, allow_enter_phase1, *factor, MAX_ITER, phase1_obj)) {
    solution_status = 4;
    return false;
  }
  ...
  if (!simplexPhase(c, allow_enter_phase2, *factor, MAX_ITER, phase2_obj)) {
...
bool LinearModel::simplexPhase(const std::vector<double> &phase_c,
                               ...
                               const int max_iter, double &phase_obj) {
