...
private:
  ...
  std::unordered_map<CashLeadtimeMultiState, std::array<double, 2>>
      cacheActions;
...
public:
  ...
  [[nodiscard]] double
  immediateValueFunction(const CashLeadtimeMultiState &state,
    ...
    const double revenue =
        prices1[t] *
            std::min(state.getIniI1() + state.getQpre1(), randomDemand1) +
        prices2[t] *
            std::min(state.getIniI2() + state.getQpre2(), randomDemand2);
    const double variableCost = unit_vari_order_costs1[t] * action1 +
                                unit_vari_order_costs2[t] * action2;
    const double inventoryLevel1 =
        state.getIniI1() + state.getQpre1() - randomDemand1;
    const double inventoryLevel2 =
        state.getIniI2() + state.getQpre2() - randomDemand2;
  ...
  stateTransitionFunction(const CashLeadtimeMultiState &state,
                          const double action1, const double action2,
                          const double randomDemand1,
                          const double randomDemand2) const {
    double nextInventory1 =
        std::max(0.0, state.getIniI1() + state.getQpre1() - randomDemand1);
    double nextInventory2 =
        std::max(0.0, state.getIniI2() + state.getQpre2() - randomDemand2);
    const double nextQpre1 = action1;
    const double nextQpre2 = action2;
    double nextCash = state.getIniCash() +
                      immediateValueFunction(state, action1, action2,
                                             randomDemand1, randomDemand2);
    nextCash = nextCash > max_cash ? max_cash : nextCash;
    nextCash = nextCash < min_cash ? min_cash : nextCash;
    nextInventory1 =
        nextInventory1 > max_inventory ? max_inventory : nextInventory1;
    nextInventory1 =
        nextInventory1 < min_inventory ? min_inventory : nextInventory1;
    nextInventory2 =
        nextInventory2 > max_inventory ? max_inventory : nextInventory2;
    nextInventory2 =
  ...
  double
  recursion(const CashLeadtimeMultiState &state) { // NOLINT(*-no-recursion)
    ...
    for (const std::vector<std::array<double, 2>> actions = feasibleActions();
      ...
      for (auto demandAndProb : pmf[state.getPeriod() - 1]) {
        thisValue += demandAndProb[2] *
                     immediateValueFunction(state, action[0], action[1],
                                            demandAndProb[0], demandAndProb[1]);
        if (state.getPeriod() < T) {
          auto newState = stateTransitionFunction(
              state, action[0], action[1], demandAndProb[0], demandAndProb[1]);
          if (cacheValues.contains(newState)) {
            // some issues here
            thisValue += demandAndProb[2] * cacheValues[newState];
          } else {
  ...
  std::vector<double> solve() {
    return {recursion(ini_state), cacheActions.at(ini_state)[0],
            cacheActions.at(ini_state)[1]};
  }
};
...
int main() {
  ...
  std::cout << "Optimal Qs in the first period is " << problem.solve()[1]
            << " and " << problem.solve()[2] << std::endl;
