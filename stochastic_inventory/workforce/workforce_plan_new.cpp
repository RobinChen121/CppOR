...
std::pair<double, double> WorkforcePlanNew::solveMIP() const {
  ...
  // 释放内存，否则会造成内存泄漏
  if (call_back == CallBack::True) {
    return mip.pieceApproximateCallback();
  }
...
std::vector<std::array<int, 2>> WorkforcePlanNew::solveMIPsS() const {
  auto sS_values =
      (call_back == CallBack::True) ? mip.get_sS_callback() : mip.get_sS(piece_segment);
  return sS_values;
}
...
//   // std::cout << "the optimality gap by MIP-sS is: " << std::fixed << std::setprecision(2) <<
//   gap3
//   //           << "%" << std::endl;
//
//   std::cout << std::string(50, '*') << std::endl;
//   const auto start_time2_2 = std::chrono::high_resolution_clock::now();
//   problem.openCallback();
//   auto [fst2, snd2] = problem.solveMIP();
//   const auto end_time2_2 = std::chrono::high_resolution_clock::now();
//   const std::chrono::duration<double> time2_2 = end_time2_2 - start_time2_2;
//   std::cout << "running time of MIP of call-back is " << time2_2.count() << 's' << std::endl;
//   const double gap1_2 = (best_value - fst2) / best_value * 100;
//   std::cout << "the optimality gap by MIP of call-back is: " << std::fixed <<
//   std::setprecision(2)
//             << gap1_2 << "%" << std::endl;
//   const double gap2_2 = snd2 / (fst2 + snd2) * 100;
//   std::cout << "the linearization gap by MIP of call-back is: " << std::fixed
//             << std::setprecision(2) << gap2_2 << "%" << std::endl;
//
//   // std::cout << std::string(50, '*') << std::endl;
//   // const auto start_time3_2 = std::chrono::high_resolution_clock::now();
//   // const auto sS_values_2 = problem.solveMIPsS();
//   // const auto end_time3_2 = std::chrono::high_resolution_clock::now();
//   // const std::chrono::duration<double> time3_2 = end_time3_2 - start_time3_2;
//   // std::cout << "running time of MIP-sS by call back is " << time3_2.count() << 's' <<
//   std::endl;
//   // std::cout << "s, S in each period are: " << std::endl;
//   // for (const auto row : sS_values_2) {
//   //   for (const auto col : row) {
//   //     std::cout << col << ' ';
//   //   }
//   //   std::cout << std::endl;
//   // }
//   // const double mip_sS_2 = problem.simulate_sS(problem.getInitialWorkers(), sS_values_2);
//   // const double gap3_2 = (mip_sS_2 - best_value) / best_value * 100;
//   // std::cout << "the optimality gap by MIP-sS of call-back is: " << std::fixed
//   //           << std::setprecision(2) << gap3_2 << "%" << std::endl;
//
//   return 0;
// }