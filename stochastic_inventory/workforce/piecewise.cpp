...
void PiecewiseWorkforce::preparePiecewiseCache(const int segment_num) const {
  ...
  for (size_t t = 0; t < T; ++t) {
    for (int j = 0; j <= static_cast<int>(t); j++) {
      ...
      const auto key = PiecewiseKey{static_cast<int>(t), j};
...
PiecewiseWorkforce::getPiecewiseResult(const int segment_num, const int t, const int j) const {
  ...
  const auto key = PiecewiseKey{t, j};
  ...
  for (int i = 0; i < segment_num; i++) {
    if (i == 0) {
      ...
      for (int j = a + 1; j <= end_x; j++) {
        ...
        if (Fy_y_minus_w(j, min_worker, p) - Fy_y_minus_w(a, min_worker, p) > 1.0 / segment_num) {
          tangent_xcoord[i] = j;
...
PiecewiseWorkforce::pieceApproximateCallback(const int segment_num) const {
  try {
    ...
    struct LazyCallback final : GRBCallback {
      ...
      LazyCallback(const PiecewiseWorkforce *self, int M, std::vector<GRBVar> *y,
      ...
      void callback() override {
        ...
        for (int t = 0; t < static_cast<int>(self->T); ++t) {
          for (int j = 0; j <= t; ++j) {
            ...
            const double u_val = getSolution((*u)[t]);
            const int min_worker = self->min_workers[t];
            double p = 1.0;
            for (int k = j; k <= t; ++k) {
              p *= 1.0 - self->turnover_rates[k];
            }
            const double loss_val = lossFunctionExpect(y_val, min_worker, 1.0 - p);
            if (loss_val - u_val < -1e-6) {
              const double slope =
                  -(1.0 - (1.0 - p)) *
                  (1 - Fy_y_minus_w(static_cast<int>(std::round(y_val)), min_worker, 1.0 - p));
              const double intercept = loss_val - slope * y_val;
              addLazy((*u)[t] >= slope * (*y)[j] + intercept + M * ((*P)[j][t] - 1));
            }
          }
        }
      }
