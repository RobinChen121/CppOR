...
NewsvendorDP::NewsvendorDP(const size_t T, const int capacity,
                           ...
                           const double unitHoldCost,
                           const double unitPenaltyCost,
    ...
    : T(static_cast<int>(T)), capacity(capacity), stepSize(stepSize),
      fixOrderCost(fixOrderCost), unitVariOrderCost(unitVariOrderCost),
      unitHoldCost(unitHoldCost), unitPenaltyCost(unitPenaltyCost),
      truncatedQuantile(truncatedQuantile), max_I(max_I), min_I(min_I),
      pmf(std::move(pmf)) {};
...
State NewsvendorDP::stateTransitionFunction(const State &state,
                                            const double action,
...
double NewsvendorDP::immediateValueFunction(const State &state,
                                            const double action,
                                            const double demand) const {
...
int main() {
  ...
  constexpr double truncQuantile =
      0.9999;                  // truncated quantile for the demand distribution
  constexpr double maxI = 500; // maximum possible inventory
  constexpr double minI = -300; // minimum possible inventory

  const auto pmf =
      PMF(truncQuantile, stepSize, distribution_type).getPMF(demands);
  const size_t T = demands.size();
  auto model1 = NewsvendorDP(T, capacity, stepSize, fixOrderCost,
                             unitVariOderCost, unitHoldCost, unitPenaltyCost,
